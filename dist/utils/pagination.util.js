...
function paginate({ list, countPerPage, query, filterException, }) {
    const { page: pg, perPage: pp, partial: pl } = query, filter = __rest(query, ["page", "perPage", "partial"]);
    ...
    if (listFilter.length != 0) {
        listPaginate = list.filter((item) => {
            const valueValid = listFilter.length;
...
function serializerStringType(query) {
    if (query == undefined) {
        return query;
    }
    ...
    const isNumber = (!query.includes(".") && Number(query)) ||
